...
def train_model(best_params, data, static_features):
...
def forecast_model(model, h):
    forecast = model.predict(h=h)
    forecast[MODEL] = forecast[MODEL].astype(int)

    return forecast

model = train_model(best_params, train, static_features)
forecast = forecast_model(model, h)
...
def export_results(forecast, file_name):
    forecast.to_csv(f'../results/{file_name}.csv', index=False)
...
if __name__ == '__main__':
    final_model = train_model(best_params, data, static_features)
    final_forecast = forecast_model(final_model, TRUE_HORIZON)

    file_name = f'forecast_full_{args.model}'

    export_results(final_forecast, file_name)
    print(f'Forecast exported to ../results/{file_name}.csv')
